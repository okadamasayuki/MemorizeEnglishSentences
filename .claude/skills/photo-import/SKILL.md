...
- 元スクショの表示: 音読タブのブロック右上の写真ボタンで、そのブロックの元スクショ(そのブロックだけを切り出した画像)を全画面表示(読み取り確認用)。`build_page_images.py parsed.json passages.json photos_dir out_dir` で `out_dir/manifest.json`(`{ブロック英文ハッシュ: 同ハッシュ}`、画像名=ハッシュ)と、各ブロックを y0/y1(上=0の正規化縦座標)でヘッダー込みに切り出し・左右下の白余白トリミング・鮮明化(UnsharpMask, quality92)した画像を生成し、`devicectl device copy to --destination Documents/page_images`(フォルダごと)で転送。アプリは PageImageStore が Documents/page_images から `<ハッシュ>.jpg` を読む(取り込み処理は不要・削除もしない)。教材画像はリポジトリに置かない
