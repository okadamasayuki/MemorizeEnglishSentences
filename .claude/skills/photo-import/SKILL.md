...
- 元スクショの表示: 音読タブのブロック右上の写真ボタンで、そのブロックの元教材ページ画像を全画面表示(読み取り確認用)。`build_page_images.py parsed.json passages.json photos_dir out_dir` で `out_dir/manifest.json`(`{ブロック英文ハッシュ: ページ番号}`)と縮小版ページ画像を生成し、`devicectl device copy to --destination Documents/page_images`(フォルダごと)で転送。アプリは PageImageStore が Documents/page_images から読む(取り込み処理は不要・削除もしない)。教材画像はリポジトリに置かない
