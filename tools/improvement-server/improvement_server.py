#!/usr/bin/env python3
"""iPhone(英語学習アプリ)の改善メモの受け口。

アプリの「改善」タブで項目をスワイプすると、ここへ HTTP で届く。
届いた要望は受信箱(~/.english-improvements/inbox.jsonl)へ記録するだけにし、
実装は、受信箱を見張っている常駐の Claude Code セッションが引き受ける。

要望ごとに Terminal で新しい claude を立ち上げる方式はやめた:
セッションごとにモデルの確認・autoモード切替・許可のやり直しが要るうえ、
GitHub への push やスマホへの配備までは面倒を見ず、同時に動くと衝突もするため。
その形に戻したいときは環境変数 ENGLISH_OPEN_TERMINAL=1 で入れ直す。

(music-3x の improvement_server.py と同じ仕組み。あちらはポート 8917、
 こちらは 8918 で、それぞれ自分のリポジトリに対して動く)
...
# 本体を保護の外(~/.english-improvements)へ写して、そちらを動かす。
# リポジトリを読むのは Terminal の中の claude なので、そちらは困らない。
REPO = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else Path(__file__).resolve().parent.parent.parent
# 要望の控えと、Terminal に渡すファイルの置き場
WORK_DIR = Path.home() / ".english-improvements"
...
def build_prompt(text: str, created_at: str) -> str:
    return (
        "iPhone の英語学習アプリ(MemorizeEnglishSentences)の「改善」タブから届いた改善要望です。"
        "リポジトリの中身を確かめて実装し、実機へ配備してください。\n"
        "\n"
        f"要望: {text}\n"
        "\n"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # 添付(写真・動画)は base64 で届く。media/ へ保存してパスだけ記録する
        saved_paths = []
        ...
        if os.environ.get("ENGLISH_OPEN_TERMINAL"):
            try:
